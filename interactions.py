...
class Interactions(object):
    """
    Interactions object. Contains (at a minimum) pair of user-item 用户物品对
    interactions. This is designed only for implicit feedback scenarios.

    Parameters
    ----------

    file_path: file contains (user,item,rating) triplets（用户，物品，评分）
    ...
    def tocoo(self):
        ...
        print('zhe si len(self)',len(self))
    ...
    def to_sequence(self, sequence_length=5, target_length=1):
        ...
        sequence_length: int
            ...
            will be left-padded with zeros.子序列小于长度，左边补0
        target_length: int
            Sequence target length.
        """

        # # change the item index start from 1 as 0 is used for padding in sequences物品索引从1开始
    ...
    def to_newsequence(self, sequence_length=5, target_length=1):
        ...
        max_sequence_length = sequence_length + sequence_length
        ...
        for i,user_id in enumerate(user_ids):
            ...
            if len(one_sequence)<=0:
                print('这是交互物品小等于0',user_id,one_sequence,indices[i])
            ...
            for train_len in range(len(one_sequence)-1):
                ...
                target_sub = one_sequence[train_len+1:train_len+1+target_length]#target_length=3


                target_len = len(target_sub)

                target_sub = np.pad(target_sub, (0, target_length-len(target_sub)), 'constant')#用0来补子目标列的长度，使它到达target_length=3
